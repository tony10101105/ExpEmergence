...
for model_name, model_data in all_data.items():
    ...
    for task, data in model_data.items():
        for d in data:
            choices = d['doc']['choices']['label']
            ans = [0 for _ in range(len(choices))]
            label = d['doc']['choices']['label'].index(d['doc']['answerKey'])      
            ans[label] = 1
...
if args.redist:
    eval_data.to_csv(f'arc_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_redist.csv', index=False)
else:
    eval_data.to_csv(f'arc_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_undist.csv', index=False)