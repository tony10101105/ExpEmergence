...
for j, idx in tqdm(enumerate(saved_question_idx)):
    ...
    for model_name, brier in saved_question_brier.items():
        ...
        eval_data.loc[model_name, f'{str(idx[0])}_{str(idx[1])}_acc'] = brier
...
if args.redist:
    eval_data.to_csv(f'acc_parsinlu_qa_mc_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_redist.csv', index=False)
else:
    eval_data.to_csv(f'acc_parsinlu_qa_mc_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_undist.csv', index=False)