...
parser.add_argument("--model_size_uplimit", default=199.52623149688787)
...
directory_path = 'abstract_narrative_understanding'
...
for model_name, model_data in all_data.items():
    ...
    for d in model_data:
        label = int(d['target'])
        ans = d['doc']['multiple_choice_scores']
        assert label == ans.index(1)
...
if args.redist:
    eval_data.to_csv(f'abstract_narrative_understanding_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_redist.csv', index=False)
else:
    eval_data.to_csv(f'abstract_narrative_understanding_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_undist.csv', index=False)