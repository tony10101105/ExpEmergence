...
parser.add_argument("--model_size_uplimit", default=63.09573444801933) # 63.09573444801933: 1.8, 39.810717055349734: 1.6, 25.118864315095795: 1.4
...
for model_name, model_data in all_data.items():
    ...
    for d in model_data:
        label = int(d['target'])
        length = d['doc']['inputs'].count('choice')
...
for k, idx in enumerate(saved_question_idx):
...
if args.redist:
    eval_data.to_csv(f'arithmetic_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_redist.csv', index=False)
else:
    eval_data.to_csv(f'arithmetic_instance_brier_{args.model_size_downlimit}_{args.model_size_uplimit}_{args.group_num}_undist.csv', index=False)