...
# parameters
dataset = 'arc' # must be in task_parameter
...
y_loc = group_plot_parameter[dataset]['y_loc_brier'] # for visulaization
...
modes = [f'{i}_{j}_brier' for i, j in saved_question_idx]

csv_file = f'data/{dataset}/{dataset}_instance_brier_-1000_{threshold_in_flops}_{group_num}_{brier_mode}.csv'
df = pd.read_csv(csv_file)
df = model_filter(df)

for m in modes:
    df = df.dropna(subset=[m])
    df[m] = df[m].apply(lambda x: float(x)*(-1)) # let brier score has the sign as accuracy
...
fig.update_layout(transition_duration=500, xaxis_title='log compute (M)', yaxis_title='TC Brier Score',
